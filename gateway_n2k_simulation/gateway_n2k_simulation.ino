...
// -------- Firmware identity --------
static const char* FW_VERSION = "1.0.2-sim";
...
static void movePoint(double latDeg, double lonDeg, double courseDeg, double distanceM, double& outLatDeg, double& outLonDeg) {
  ...
  const double lat2 = asin(sinLat1 * cosDr + cosLat1 * sinDr * cos(brng));
  const double lon2 = lon1 + atan2(sin(brng) * sinDr * cosLat1, cosDr - sinLat1 * sin(lat2));
...
static void appendI16LE(std::vector<uint8_t>& v, int16_t x) {
  appendU16LE(v, (uint16_t)x);
}
...
static void appendI32LE(std::vector<uint8_t>& v, int32_t x) {
  appendU32LE(v, (uint32_t)x);
}
...
static void appendI64LE(std::vector<uint8_t>& v, int64_t x) {
  appendU64LE(v, (uint64_t)x);
}

static uint16_t rad1e4(double deg) {
  double rad = degToRad(deg);
  long val = lround(rad * 10000.0);
  if (val < 0) val = 0;
...
static uint16_t speedCms(double knots) {
  double mps = knots * KNOT_TO_MPS;
  long val = lround(mps * 100.0);
  if (val < 0) val = 0;
  if (val > 65534) val = 65534;
  return (uint16_t)val;
}
...
static void printSummary(uint32_t nowMs) {
  ...
  Serial.printf(
    ...
    (unsigned long)gSimFramesGenerated,
...
static void notifyPGN(uint32_t pgn, uint8_t src, const uint8_t* payload, size_t len, bool is_ais) {
  ...
  if (LOG_NOTIFY_FLOW) {
    Serial.printf("NOTIFY SEND PGN=%u SRC=%u framedLen=%u data=", (unsigned)pgn, (unsigned)src, (unsigned)(len + 1));
...
static std::vector<uint8_t> buildPGN129026_CogSog(double cogDeg, double sogKt) {
  std::vector<uint8_t> p;
  appendU8(p, 0x00);               // SID
  appendU8(p, 0xFC);               // COG reference=true
  appendU16LE(p, rad1e4(cogDeg));  // COG, radians x 1e-4
  appendU16LE(p, speedCms(sogKt)); // SOG, m/s x 100
...
static std::vector<uint8_t> buildPGN127250_Heading(double headingDeg, double variationDeg) {
  std::vector<uint8_t> p;
  appendU8(p, 0x00);                         // SID
  appendU16LE(p, rad1e4(headingDeg));        // Heading
  appendI16LE(p, signedRad1e4(0.0));         // Deviation
  appendI16LE(p, signedRad1e4(variationDeg));// Variation
  appendU8(p, 0xFC);                         // Reference=true
  return p;
}
...
static std::vector<uint8_t> buildPGN128259_WaterSpeed(double waterSpeedKt) {
  ...
  appendU16LE(p, 0xFFFF);               // ground referenced speed unavailable here
  appendU8(p, 0xFF);
  appendU8(p, 0xFF);
  appendU8(p, 0xFF);
  return p;
...
static std::vector<uint8_t> buildPGN128267_Depth(double depthM, double offsetM) {
  std::vector<uint8_t> p;
  appendU8(p, 0x00);                         // SID
  appendU32LE(p, (uint32_t)lround(depthM * 100.0)); // depth, 0.01 m
  appendI16LE(p, (int16_t)lround(offsetM * 1000.0)); // offset, 0.001 m
  appendU8(p, 0xFF);
  return p;
}
...
static std::vector<uint8_t> buildPGN130306_Wind(double windSpeedKt, double windAngleDeg) {
  std::vector<uint8_t> p;
  appendU8(p, 0x00);                      // SID
  appendU16LE(p, speedCms(windSpeedKt));  // wind speed m/s x100
  appendU16LE(p, rad1e4(windAngleDeg));   // wind angle rad x1e-4
  appendU8(p, 0xFD);                      // apparent wind-ish reference
  appendU8(p, 0xFF);
  appendU8(p, 0xFF);
  return p;
}
...
static std::vector<uint8_t> buildPGN130310_Env(double waterTempC, double airTempC, double pressurePa) {
  std::vector<uint8_t> p;
  appendU8(p, 0x00);                         // SID
  appendU16LE(p, tempK_0p01C(waterTempC));   // water temp
  appendU16LE(p, tempK_0p01C(airTempC));     // outside ambient temp
  appendU16LE(p, (uint16_t)lround(pressurePa / 100.0)); // pressure, hPa-ish compact legacy
  appendU8(p, 0xFF);
  return p;
}
...
static std::vector<uint8_t> buildPGN130311_EnvOld(double tempC, double humidityPct, double pressurePa) {
  std::vector<uint8_t> p;
  appendU8(p, 0x00);                       // SID
  appendU8(p, 0x00);                       // temp source / humidity source packed-ish
  appendU16LE(p, tempK_0p01C(tempC));
  appendU16LE(p, (uint16_t)lround(humidityPct * 250.0)); // relative humidity approx 0.004%
...
static std::vector<uint8_t> buildPGN130312_Temperature(double tempC) {
  std::vector<uint8_t> p;
  appendU8(p, 0x00);                     // SID
  appendU8(p, 0x00);                     // instance
  appendU8(p, 0x01);                     // source = sea temp-ish
  appendU16LE(p, tempK_0p01C(tempC));
  appendI16LE(p, 0x7FFF);                // set temp unavailable
  appendU8(p, 0xFF);
  return p;
}
...
static std::vector<uint8_t> buildPGN130314_Pressure(double pressurePa) {
  ...
  appendU8(p, 0x00);                          // SID
  appendU8(p, 0x00);                          // instance
  appendU8(p, 0x00);                          // source atmospheric
  appendU32LE(p, pressurePa_0p1(pressurePa)); // pressure, 0.1 Pa
  appendU8(p, 0xFF);
  return p;
}
...
static std::vector<uint8_t> buildPGN130316_TempExtended(double tempC) {
  ...
  appendU8(p, 0x00);                    // SID
  appendU8(p, 0x00);                    // instance
  appendU8(p, 0x01);                    // source
  appendI32LE(p, (int32_t)lround((tempC + 273.15) * 1000.0)); // K x 0.001
  appendU8(p, 0xFF);
  return p;
}
...
static std::vector<uint8_t> buildPGN127505_Fluid(double levelPct, double capacityL) {
  std::vector<uint8_t> p;
  appendU8(p, 0x00);                           // instance/type-ish
  appendU16LE(p, (uint16_t)lround(levelPct * 250.0)); // 0.004%
  appendU32LE(p, (uint32_t)lround(capacityL * 10.0)); // 0.1 L
  appendU8(p, 0xFF);
  return p;
}
...
static std::vector<uint8_t> buildPGN127508_Battery(double volts, double amps, double tempC) {
  std::vector<uint8_t> p;
  appendU8(p, 0x00); // battery instance
...
static std::vector<uint8_t> buildPGN127506_DcDetailed(double volts, double amps, double socPct) {
  std::vector<uint8_t> p;
  appendU8(p, 0x00); // SID
  appendU8(p, 0x00); // DC instance
  appendU8(p, 0x01); // battery instance
  appendU8(p, 0xFC); // type/status packed-ish
  appendU16LE(p, (uint16_t)lround(socPct * 250.0));
  appendU16LE(p, (uint16_t)lround(95.0 * 250.0)); // SOH
  appendU16LE(p, (uint16_t)lround(60.0 * 60.0));  // time remaining, minutes-ish placeholder
  appendU16LE(p, (uint16_t)lround(0.0));
  appendU16LE(p, (uint16_t)lround(volts * 100.0));
  appendI16LE(p, (int16_t)lround(amps * 10.0));
  return p;
}
...
static std::vector<uint8_t> buildPGN127488_EngineRapid(double rpm) {
  std::vector<uint8_t> p;
  appendU8(p, 0x00); // engine instance
  appendU16LE(p, (uint16_t)lround(rpm * 4.0)); // rpm, 0.25 rpm
  appendU16LE(p, (uint16_t)lround(25.0 * 100.0)); // boost pressure placeholder
  appendI8(p, 0);
  appendU8(p, 0xFF);
  appendU8(p, 0xFF);
  return p;
...
static std::vector<uint8_t> buildPGN127489_EngineDynamic(double oilPressurePa, double coolantTempC, double alternatorV, double fuelRateLph, double hours) {
  std::vector<uint8_t> p;
  appendU8(p, 0x00); // engine instance
  appendU16LE(p, (uint16_t)lround(oilPressurePa / 100.0));
  appendU16LE(p, tempK_0p01C(coolantTempC));
  appendU16LE(p, tempK_0p01C(65.0)); // alternator temp placeholder
  appendU16LE(p, (uint16_t)lround(alternatorV * 100.0));
  appendI16LE(p, (int16_t)lround(6.5 * 10.0)); // fuel pressure placeholder
  appendI16LE(p, (int16_t)lround(fuelRateLph * 10.0));
  appendU32LE(p, (uint32_t)lround(hours * 3600.0));
  appendU16LE(p, 0x0000); // coolant pressure
  appendU16LE(p, 0xFFFF); // fuel temp unavailable
  appendU8(p, 0x00);
  appendU8(p, 0x00);
  return p;
}

static std::vector<uint8_t> buildPGN127493_Transmission(double oilPressurePa, double oilTempC) {
  std::vector<uint8_t> p;
  appendU8(p, 0x00); // instance
  appendU8(p, 0x00); // gear = neutral/forward-ish placeholder
  appendU16LE(p, (uint16_t)lround(oilPressurePa / 100.0));
  appendU16LE(p, tempK_0p01C(oilTempC));
  appendU8(p, 0x00); // status 1
  appendU8(p, 0x00); // status 2
  return p;
}
...
static std::vector<uint8_t> buildPGN127245_Rudder(double rudderDeg) {
  std::vector<uint8_t> p;
  appendU8(p, 0x00);                         // instance
  appendU8(p, 0xFF);                         // direction/order unavailable
  appendI16LE(p, signedRad1e4(rudderDeg));
  appendI16LE(p, 0x7FFF);                    // order unavailable
  appendU8(p, 0xFF);
  appendU8(p, 0xFF);
  return p;
}
...
static std::vector<uint8_t> buildPGN126992_SystemTime(uint32_t nowMs) {
  // Simplified date/time that advances from an arbitrary base date.
  const uint32_t simSeconds = nowMs / 1000;
  const uint16_t daysSince1970 = 20000 + (simSeconds / 86400);
  const uint32_t secondsToday = simSeconds % 86400;

  std::vector<uint8_t> p;
  appendU8(p, 0x00); // SID
  appendU8(p, 0x00); // source = GPS-ish
  appendU16LE(p, daysSince1970);
  appendU32LE(p, secondsToday * 10000UL); // 0.0001 s
  return p;
}

static std::vector<uint8_t> buildPGN129029_GNSS(double latDeg, double lonDeg, double altitudeM) {
  std::vector<uint8_t> p;
  appendU8(p, 0x00); // SID
  appendU16LE(p, 20000); // days since 1970 placeholder
  ...
  appendI64LE(p, (int64_t)llround(altitudeM * 1e6));
  appendU8(p, 0x01);   // GNSS type GPS
  appendU8(p, 0x00);   // method/no integrity-ish
  appendU8(p, 12);     // number of SVs
  appendI16LE(p, (int16_t)lround(0.8 * 100.0)); // HDOP
  appendI16LE(p, (int16_t)lround(1.2 * 100.0)); // PDOP
  appendI32LE(p, 0);   // geoidal separation
  appendU8(p, 0);      // ref stations
  return p;
}
...
static std::vector<uint8_t> buildAISDynamic(uint32_t mmsi, double latDeg, double lonDeg, double cogDeg, double sogKt, double headingDeg) {
  std::vector<uint8_t> p;
  appendU8(p, 0x00);                         // message ID/repeat/SID-ish placeholder
  appendU32LE(p, mmsi);
  appendI32LE(p, (int32_t)llround(latDeg * 1e7));
  appendI32LE(p, (int32_t)llround(lonDeg * 1e7));
  ...
  appendU8(p, 0xFF);                         // nav status / accuracy placeholder
  appendU8(p, 0xFF);
  appendU8(p, 0xFF);
  return p;
}
...
static std::vector<uint8_t> buildAISClassBStaticB(uint32_t mmsi, uint8_t shipType) {
  ...
  appendU32LE(p, mmsi);
  appendU8(p, shipType);
  appendFixedText(p, "MAPTATTOO SIM", 20); // vendor/callsign-like placeholder
  return p;
}
...
static std::vector<uint8_t> buildAISClassAStaticVoyage(uint32_t mmsi, const char* name, uint8_t shipType) {
  ...
  appendU8(p, 0x00);
  appendU32LE(p, mmsi);
  appendU32LE(p, 9876543);       // IMO placeholder
  appendFixedText(p, "WZZ9999", 7);
  appendFixedText(p, name, 20);
  appendU8(p, shipType);
  appendU16LE(p, 300);           // length 30.0m-ish if receiver interprets x0.1
  appendU16LE(p, 80);            // beam 8.0m-ish
...
static std::vector<uint8_t> buildAtoN(uint32_t mmsi, double latDeg, double lonDeg) {
  ...
  appendI32LE(p, (int32_t)llround(latDeg * 1e7));
  appendI32LE(p, (int32_t)llround(lonDeg * 1e7));
  appendFixedText(p, "MONTEREY AtoN", 20);
  appendU8(p, 0x00);
  return p;
}
...
static void getOwnship(double tSec, double& lat, double& lon, double& cogDeg, double& sogKt, double& headingDeg) {
...
static void simulateOwnship1Hz(uint32_t nowMs) {
  ...
  const double yawDeg = headingDeg;
  ...
  p = buildPGN127257_Attitude(yawDeg, pitchDeg, rollDeg); notifyPGN(127257, SRC_OWNSHIP_NAV, p.data(), p.size(), false); gSimFramesGenerated++;
...
static void simulateAis10Hz(uint32_t nowMs) {
  static uint32_t lastMs = 0;
  ...
  for (uint8_t i = 0; i < 15; ++i) {
    ...
    double lat = a.lat0;
    double lon = a.lon0;
    double cog = wrap360(a.cogDeg + oscillate(0.0, 3.0, 75.0 + i * 3.0, i, tSec));
    ...
    if (i == 12) {
      // One fixed simulated AtoN.
      p = buildAtoN(a.mmsi, a.lat0, a.lon0);
    ...
    p = buildAISDynamic(a.mmsi, lat, lon, cog, sog, wrap360(cog + oscillate(0.0, 2.0, 21.0, i, tSec)));
    notifyPGN(dynPgn, src, p.data(), p.size(), true);
    gSimFramesGenerated++;

    // Use Class B extended position for a few class B vessels.
    if (!a.classA && (i % 4 == 0)) {
...
static void simulateAisStatic30s(uint32_t nowMs) {
  static uint32_t lastMs = 0;
  if (nowMs - lastMs < 30000 && lastMs != 0) return;
  lastMs = nowMs;
...
void setup() {
  ...
  Serial.println("AIS: 15 simulated Monterey Bay targets.");
}
...
void loop() {
  ...
  // Generate data continuously, like a live N2K network would.
